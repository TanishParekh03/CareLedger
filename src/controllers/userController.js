...
const signUpUser = async (req, res, next) => {
    try {
        ...
        const checkUser = await pool.query(
            'SELECT id FROM users WHERE email = $1 AND phone = $2 AND role = $3',
            [email, phone, role]
        );
        ...
        const result = await pool.query(
            'INSERT INTO users (email, phone, hashed_password, role) VALUES ($1, $2, $3, $4) RETURNING email, phone, role',
...
const userLogin = async (req, res, next) => {
    try {
        ...
        const passwordMatch = await bcrypt.compare(plain_password, user.hashed_password);
        if (!passwordMatch) {
...
const updateUser = async (req, res, next) => {
    try {
        ...
        if (plain_password) {
            const hashed_password = await bcrypt.hash(plain_password, 12);
            updateQuery += `hashed_password = $${paramCount}, `;
